...
export const query = graphql`
  query($path: String!, $lang: String!) {
    ...
    productos: allPwPages(
      filter: { page_template: { eq: "PRODUCTO" }, lang: { eq: $lang } }
    ) {
      nodes {
        page_url
    ...
    categorias: allSitePage(
      filter: {
        context: { page_template: { eq: "CATEGORIA" }, lang: { eq: $lang } }
      }
    ) {
      nodes {
        context {
...
const ProductosTemplate = props => {
  ...
  const selectOptions = _.map(categorias, categoria => ({
    value: categoria.context.page_url,
    label: categoria.context.title,
  }))
  selectOptions.unshift({
    value: props.data.productoPage.page_url,
    label: translations.elijecategoria[props.pageContext.lang],
  })
      ...
      <div className="content_layout">
        ...
        <div>
          <h2 className={s.titulo}>
            {translations.productos[props.pageContext.lang]}
          </h2>
          <p className={s.intro}>
            {translations.cuandolacalidadimporta[props.pageContext.lang]}
          </p>
          <Select options={selectOptions} currentPage={props.path} />
          <div className={s.productos_listado}>
            {productosFiltrados.map((producto, i) => (
              <CLink
                to={producto.page_url}
                key={i}
                className={s.producto_ficha}
              >
                <div className={s.image_container}>
                  {producto.producto_imagen_thumb.image ? (
                    <Img
                      fluid={
                        producto.producto_imagen_thumb.image.childImageSharp
                          .fluid
                      }
